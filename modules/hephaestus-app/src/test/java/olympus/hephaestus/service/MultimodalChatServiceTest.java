...
import olympus.hephaestus.media.exception.MediaStorageException;
...
class MultimodalChatServiceTest {
    ...
    void returnsRemoteImageUrlWhenImageStorageDownloadFails() {
        ...
        when(responseSpec.content()).thenReturn(Flux.just(
                "{\n"
                        + "  \"generateImage\": true,\n"
                        + "  \"reply\": \"好的，我来为你生成一张慵懒的猫咪图片。\",\n"
                        + "  \"imagePrompt\": \"一只慵懒的猫咪躺在沙发上睡觉，温暖午后，插画风格\"\n"
                        + "}"
        ));
        ...
        MultimodalChatService.MultimodalResponse response = service.chat("画一只慵懒的猫咪", null, "session-404");

        assertThat(response.type()).isEqualTo("IMAGE");
        assertThat(response.generateImage()).isTrue();
        assertThat(response.reply()).isEqualTo("好的，我来为你生成一张慵懒的猫咪图片。");
    ...
    void fallsBackToInlineImageWhenImageModelFails() {
        ...
        MediaFile savedFile = new MediaFile(
                ...
                128L,
                "rec/down/20260519/session-inline/9f645494-9e1b-4b82-8fd4-b9f1f11bf51d/generated-image.svg",
                "/mediadl/media/getdata/rec/down/20260519/session-inline/9f645494-9e1b-4b82-8fd4-b9f1f11bf51d/generated-image.svg",
                "AI_GENERATED_IMAGE",
                null
        );
        when(mediaFileService.save(eq("session-inline"), eq(storedMediaFile), eq("AI_GENERATED_IMAGE"), anyString()))
