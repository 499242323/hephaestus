...
- 前端在设置抽屉中选择“当前人员”
- 后续组织管理请求统一携带 `X-Person-Id`
- 后端通过 `X-Person-Id` 查出当前人员所属 `unit`
...
- 未提供 `X-Person-Id` 时，返回明确错误
- `X-Person-Id` 对应人员不存在时，返回明确错误
...
- 当前没有正式登录态，`X-Person-Id` 方案适合第一版联调，但不是最终鉴权方案
